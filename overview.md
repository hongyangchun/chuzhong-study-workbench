...
# 记忆 tab 层级修正：复习/浏览提升为顶层

## 背景
用户指出「复习/浏览」应比「古诗文/词汇/记忆卡」处于**更高的层级**。原 DOM 顺序是子库标签在上、模式切换器在下，视觉上子库反而成了上级，与「复习是顶层视图选择、子库是下级选择」的认知相反。

## 改动（commit `313ee37`，已 push，Cloudflare 上线）
- 把 `.mem-toolbar`（模式切换器 复习/浏览）从子库标签下方移到**上方**，成为记忆 tab 的顶层视图选择。
- 子库选择（古诗文/词汇/记忆卡）包进新 `.mem-sublib-row`，并加「子库」小标签（`.mem-sublib-label`，12px muted），显式标出次级层级。
- 现在结构：**视图 [复习 | 浏览]（上级）→ 子库 [古诗文 | 词汇 | 记忆卡]（下级）→ 内容**。
- `applyMemModeVisibility` 不动 toolbar/子库行，仅切换看板与子库内容区，逻辑无影响；复习态下工具栏只显示模式钮，浏览态显示模式钮+筛选+新增。

## 验证
- 内联脚本 `node --check` 通过
- grep 确认 `class="mem-toolbar"`(1446) 行号 < `class="memory-tab-header"`(1470，在 `mem-sublib-row` 内) → 顺序反转成功；`mem-sublib-row`/`mem-sublib-label` 的 CSS+HTML 各 ≥1
- 线上 curl 经 `sleep 45` 多轮稳定：`mem-sublib-row=2 / mem-sublib-label=2`
